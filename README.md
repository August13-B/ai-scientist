...
- 研究方向：通用领域科学假设生成（面向任意科研学科，支持领域偏好；当前代表案例选「深度学习 × 固态硬盘（SSD）存储」）
...
| ④ 智能体思辨与人在回路 | 倡议者/质疑者结构化辩论；假设生成后系统暂停，人类导师可介入修改后继续 |
...
**调用链**：用户提问 → 问题理解 Agent 拆解子查询 → 并行执行（文献检索/知识发现）→ 假设生成 Agent → 人在回路暂停点 → 评估 Agent（评分 + 幻觉检测）→ 实验设计 Agent → 辩论 Agent → 输出《科学假设与研究计划》。
...
| AI 编排 | LangChain4j 1.18.1 | DAG 编排、@Tool/@AiService、State 管理 |
